...
const Connexion = ({ clickInscription }: { clickInscription: () => void }) => {
  ...
  const pseudo = useSelector(
    (state: RootState) => state.userReducer.pseudoInput
  );
  const password = useSelector(
    (state: RootState) => state.userReducer.passwordInput
  );
  ...
  return (
    <div className='md:w-1/5 h-fit w- bg-white text-center p-3 flex flex-col absolute z-10'>
      <form action='#' className='flex flex-col'>
        <p className='text-black'>Connection</p>
        <label htmlFor='pseudo' className='text-black'></label>
        <input
          ...
          value={pseudo}
          onChange={(event) =>
            dispatch(actionSetPseudoInput(event.target.value))
          }
        />
        ...
        <input
          ...
          value={password}
          onChange={(event) =>
            dispatch(actionSetPasswordInput(event.target.value))
          }
        />
        <button
      ...
      <button
        ...
        className='bg-sky-400 px-3 py-1 rounded-lg hover:bg-sky-500 text-black'
        type='button'>
        Inscription
      </button>
    </div>
