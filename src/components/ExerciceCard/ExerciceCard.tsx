...
export const ExerciceCard = () => {
  ...
  const filteredExercices = exercices.filter((exercice) => {
    const isFullBody =
      buttonStates[0] && exercice.category.name === "Full body";
    const isHautDuCorps =
      buttonStates[1] && exercice.category.name === "Haut du corps";
    const isBasDuCorps =
      buttonStates[2] && exercice.category.name === "Bas du corps";
  ...
  return (
    <div className='exercice-container'>
      ...
      <h5 className='text-1xl text-gray-800 italic mx-4 md:mx-16 mb-4 md:mb-10 mt-6'>
        Explorez notre page d'exercices accompagnée d'instructions détaillées et
        d'images explicatives qui vous guideront vers une exécution parfaite.
        Parce que bien effectuer chaque mouvement est non seulement essentiel
        pour des résultats efficaces, mais aussi pour votre bien-être!
      </h5>
      ...
      {filteredExercices.map((exercice, index) => (
        <div
          ...
          <div className='w-full md:w-1/2 p-4 text-center text-gray-600'>
